...
fn candidates(product: &str, configured_path: Option<&str>) -> Vec<PathBuf> {
    ...
    for root in roots {
        #[cfg(target_os = "macos")]
        {
            ...
            files.push(app.join("Contents/Info.plist"));
...
fn windows_file_version(path: &Path) -> Option<String> {
    ...
    let out = std::process::Command::new("powershell").creation_flags(0x08000000).args(["-NoProfile", "-NonInteractive", "-ExecutionPolicy", "Bypass", "-Command", "$p=$env:COCKPIT_CLIENT_EXE;$v=(Get-Item $p).VersionInfo;$value=$v.ProductVersion;if([string]::IsNullOrWhiteSpace($value)){$value=$v.FileVersion};Write-Output $value"]).env("COCKPIT_CLIENT_EXE", path).output().ok()?;
    if out.status.success() {
...
mod tests {
    ...
    #[test]
    fn detects_product_and_language_server_versions_from_fixture() {
        use std::fs;
        ...
        let suffix = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let root = std::env::temp_dir().join(format!("cockpit-version-fixture-{}.app", suffix));
        #[cfg(target_os = "macos")]
        let base = root.join("Contents");
        #[cfg(not(target_os = "macos"))]
        let base = root.clone();
        let product_dir = base.join("Resources/app");
        let host_dir = product_dir.join("out/vs/workbench/api/node");
        fs::create_dir_all(&host_dir).unwrap();
        fs::write(
            product_dir.join("product.json"),
            r#"{"windsurfVersion":"3.0.28","version":"1.110.1"}"#,
        )
        .unwrap();
        fs::write(
            host_dir.join("extensionHostProcess.js"),
            r#"var F6="1.48.2";var yw=class{get languageServerVersion(){return F6}}"#,
        )
        .unwrap();
        let metadata = super::detect_windsurf_version_metadata("Fixture", root.to_str());
        assert_eq!(metadata.ide_version.as_deref(), Some("3.0.28"));
        assert_eq!(metadata.extension_version.as_deref(), Some("1.48.2"));
