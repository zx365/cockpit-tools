...
// CodeBuddy 网关会拒绝缺少 User-Agent 的请求（HTTP 403 / code=10085）。
// 官方桌面端通过 Chromium 发起请求，使用浏览器风格 UA。
const CODEBUDDY_HTTP_USER_AGENT: &str =
...
fn build_client() -> Result<reqwest::Client, String> {
    reqwest::Client::builder()
        ...
        .user_agent(CODEBUDDY_HTTP_USER_AGENT)
        .build()
...
async fn start_login_with_platform(platform: &str) -> Result<CodebuddyOAuthStartResponse, String> {
    ...
    let configured_path = crate::modules::config::get_user_config().codebuddy_cn_app_path;
    let version = tokio::task::spawn_blocking(move || {
        crate::modules::client_version::detect_client_version(
            "CodeBuddy CN",
...
///
/// 网页端企业用量走 `POST /v2/billing/meter/get-enterprise-user-usage`，
...
mod tests {
    ...
    #[test]
    fn current_quota_contract_and_response_shapes_are_supported() {
        let body = build_user_resource_request_body();
        ...
        let wrapped = wrap_enterprise_usage_as_resource(&json!({
            "data": { "data": { "limitNum": "200", "credit": "50", "cycleResetTime": 1_800_000_000 } }
        }))
        .unwrap();
        let resource = wrapped.pointer("/data/Response/Data/Accounts/0").unwrap();
        assert_eq!(resource["CycleCapacityRemainPrecise"], "150");
        assert_eq!(resource["CycleResetTime"], "1800000000");
    }

    #[test]
    fn enterprise_unlimited_and_token_expiry_are_preserved() {
        let wrapped = wrap_enterprise_usage_as_resource(&json!({
            "data": { "data": { "limit_num": -1, "used_num": 12 } }
        }))
        .unwrap();
        assert_eq!(
            wrapped.pointer("/data/Response/Data/Accounts/0/Unlimited"),
            Some(&json!(true))
        );
        assert_eq!(
